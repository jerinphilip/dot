...
beautiful.wallpaper = "/home/jerin/Dropbox/wallpaper/GOT/old_gods_by_montezuma3-d5hlef9.jpg"
-- {{{ Wallpaper
...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "1:main", "2:web", "3:im", "4:code", "5:media", 6, 7, 8, 9 }, s, layouts[1])
end
-- }}}
...
globalkeys = awful.util.table.join(
	...
	awful.key({ modkey }, "g", function() awful.util.spawn("gnome-terminal") end),
	awful.key({ modkey }, "v", function() awful.util.spawn("vlc") end),
	awful.key({ modkey, "Shift"}, "p", function() awful.util.spawn("evince") end),


	--Hardware Buttons
	awful.key({}, "#107", function() take_screenshot() end),
	awful.key({}, "#121", function() toggle_sound() end),
	awful.key({}, "#122", function() change_sound("3%-") end),
	awful.key({}, "#123", function() change_sound("3%+") end),
	awful.key({}, "#232", function() change_brightness(-10) end),
	awful.key({}, "#233", function() change_brightness( 10) end),
...
--Startup Signal
local x = awful.util.pread("systemd-analyze | awk -F '=' '{print $2}'")
local un = string.gsub( awful.util.pread("whoami"), "\n", "")
naughty.notify({title="Hello, "..un.."!", text="Startup completed in"..x})


--Battery Function
function check_battery()
	local battery_percent = awful.util.pread("acpi | awk -F ',' '{print $2}'|sed 's/%//'")
	local battery_val = tonumber(battery_percent)
	if battery_val < 10 then
        if battery_notification ~= nil then naughty.destroy(battery_notification) end
		battery_notification = naughty.notify({title="Battery Low",
						preset=naughty.config.presets.critical,
						text=battery_val.."% Remaining"})
	end
end
